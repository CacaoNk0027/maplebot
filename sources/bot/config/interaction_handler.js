...
async function load_interactions() {
    if (cachedMenus)
        return cachedMenus;
    cachedMenus = await load_handlers('menus', 'interaction');
    return cachedMenus;
}
async function load_modals() {
    if (cachedModals)
        return cachedModals;
    ...
    try {
        ...
        for (let file of files) {
            try {
                ...
                handlers.set(handler.data.id, handler);
            }
            catch (error) {
                console.error(`Error al cargar ${folder}/${file}:`, error);
            }
        }
        return handlers;
    }
    catch (error) {
        console.error(`Error al cargar ${folder}:`, error);
        throw error;
    }
}
