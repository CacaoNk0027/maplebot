...
const command = {
    data: new command_data_1.default()
        .setName('kill')
        .setAliases('matar', 'asesinar')
        .setDescription('Acaba con la vida de alguien')
        .setId('011', '005')
        .setCooldown(5)
        .ignoreSlash()
        .validForLeveling()
        ...
        .setName('user')
        .setRequired(true)
        .setDescription('Menciona al usuario para matarlo')),
    exec: async () => {
        return;
    },
...
async function execute(target, args) {
    let kill = new action_1.default(target, {
        args,
        action: 'kill'
    })
        ...
        return [
            `**${names.for_author}** ha acabado con la vida de **${names.for_user}**`,
            `**${names.for_author}** asesina a **${names.for_user}**`,
            `**${names.for_author}** ha asesinado a **${names.for_user}**`,
            `**${names.for_author}** apaga la vida de **${names.for_user}**`
        ];
    });
    (await kill.add_user()).execute();
}
