...
const modal = {
    data: new interaction_data_1.default().setId('modal.006'),
    async exec(interaction) {
        const selected = interaction.fields.getStringSelectValues('menu.004.006')[0];
        const value = interaction.fields.getTextInputValue('menu.004.006.value').trim();
        if (!selected || !interaction.guildId) {
            await interaction.reply({
                content: (0, config_1.reply)('error', 'Hubo un error al intentar guardar el fondo de bienvenida.'),
                flags: ['Ephemeral']
            });
            return;
        }
        ...
        try {
            await Welcome_1.default.setBackground(interaction.guildId, type, backgroundValue);
        }
        catch (error) {
            console.error('Error al guardar el fondo de bienvenida:', error);
            await interaction.reply({
                content: (0, config_1.reply)('error', 'Hubo un error al intentar guardar el fondo de bienvenida.'),
                flags: ['Ephemeral']
            });
