...
const modal = {
    data: new interaction_data_1.default().setId('modal.014'),
    async exec(interaction) {
        const title = interaction.fields.getTextInputValue('menu.005.007.text').trim();
        const description = interaction.fields.getTextInputValue('menu.005.007.background').trim();
        const border = interaction.fields.getTextInputValue('menu.005.007.border').trim();
        ...
        try {
            await Farewell_1.default.setColors(interaction.guildId, colors);
        }
        catch (error) {
            console.error('Error al guardar los colores de despedida:', error);
            await interaction.reply({
                content: (0, config_1.reply)('error', 'Hubo un error al intentar guardar los colores de despedida.'),
                flags: ['Ephemeral']
            });
            return;
        }
        ...
        await interaction.reply({
            embeds: [{
                    description: (0, config_1.reply)('ok', 'Se actualizaron los colores de despedida.'),
