    ...
    try {
        ...
        let channel; try {
            channel = (await client.channels.fetch((
                await config.schemas.SetChannels.findOne({ guildID: message.guildId }).exec().then(obj => obj.suggest)
            )))
        } catch (error) {
            ...
            if (embed.footer.text !== "Code: cmVzcG9uZGlkbw==") {
                ...
                await message.channel.send({
                    embeds: [{
                        ...
                        color: 0xff0000,
                        description: models.utils.statusError('success', `La sugerencia se ha declinado con la siguente razon\n> ${reason}\n`)
                    }]
                })
...
exports.help = {
    ...
    alias: ['declinesug'],
    id: '033',
    description: 'declina una sugerencia del servidor',
    category: 'moderacion',
    options: [{
        name: "mensaje id",
        required: true
