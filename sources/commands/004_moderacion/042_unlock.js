...
exports.text = async (client, message, args) => {
    try {
        ...
        if(channel.permissionsFor(message.guild.roles.everyone).has(discord.PermissionFlagsBits.SendMessages)) return await message.reply({
            embeds: [{
                description: config.statusError('error', `parece que el canal <#${channel.id}> ya esta desbloqueado`),
                color: 0xff0000
            }]
        })
        await message.channel.permissionOverwrites.edit(message.guild.roles.everyone, {
            ...
            await message.reply({
                embeds: [{
                    description: config.statusError('success', `El canal <#${channel.id}> ha sido desbloqueado correctamente`),
                    color: 0x00ff00
                }]
            })
        }).catch(err => err)
